...
namespace Ustad.API.Controllers
{
    ...
    {
        private readonly ESrcExternalDataService _eSrcExternalDataService;
        private readonly StudentDataService _studentDataService;
        private readonly StudentBalanceService _balanceService;
        private readonly ActivityTimelineService _activityTimelineService;
        private readonly ESrcExternalDataCacheService _cacheService;
        private readonly ILogger<ESrcExternalDataController> _logger;

        public ESrcExternalDataController(
            ESrcExternalDataService eSrcExternalDataService,
            StudentDataService studentDataService,
            StudentBalanceService balanceService,
            ActivityTimelineService activityTimelineService,
            ESrcExternalDataCacheService cacheService,
            ILogger<ESrcExternalDataController> logger)
        {
            _eSrcExternalDataService = eSrcExternalDataService;
        ...
        public async Task<IActionResult> SyncStudent([FromBody] ESrcExternalDataSyncRequest request)
        {
            try
            {
                ...
                if (request.StudentId.HasValue)
                {
                    ...
                    studentData = await _studentDataService.GetStudentByIdAsync(studentId.Value);
                    
                    if (studentData == null)
                    {
                        return BadRequest($"Student with ID {studentId} not found or inactive");
                    }
                ...
                // Step 2: Check cache
                var cachedResponse = _cacheService.GetCachedResponse(studentId, tcNo);
                if (cachedResponse != null)
                {
                    _logger.LogInformation(
                        "[ESrcExternalDataController] Returning cached response for student {StudentId}",
                        studentId);
                ...
                {
                    balance = await _balanceService.CalculateBalanceAsync(studentId.Value);
                }
                else if (studentData.BAKIYE.HasValue)
                {
                    balance = studentData.BAKIYE.Value;
                }

                // Step 4: Build remote API model
                var remoteApiModel = _eSrcExternalDataService.BuildRemoteApiModel(studentData, balance);

                // Step 5: Call e-src.net service with retry
                var syncResponse = await _eSrcExternalDataService.SyncStudentAsync(remoteApiModel, studentId, tcNo);

                // Step 6: Log to activity timeline (if we have student ID)
                if (studentId.HasValue)
                {
                    string status = syncResponse.Success ? "success" : "failure";
                    string message = syncResponse.Success
                        ? $"Successfully synced with e-src.net"
                        : $"Failed to sync with e-src.net: {syncResponse.Message}";
                    
                    string metadata = $"{{" +
                        $"\"retryAttempts\": {syncResponse.RetryAttempts}, " +
                        $"\"fromCache\": false, " +
                        $"\"errorDetails\": \"{syncResponse.ErrorDetails ?? "none"}\"" +
                        $"}}";

                    await _activityTimelineService.LogESrcExternalDataSyncAsync(
                        studentId.Value,
                        status,
                        message,
                        metadata);
                ...
                // Step 7: Cache response (both success and failure)
                _cacheService.SetCachedResponse(studentId, tcNo, syncResponse);
        ...
        public IActionResult InvalidateCache([FromQuery] int? studentId, [FromQuery] string? tcNo)
        {
            try
            {
                ...
                _cacheService.InvalidateCache(studentId, tcNo);
